...
def candlestick_with_levels(candles: pd.DataFrame, entry: float | None = None,
    ...
    for value, name, color in (
        (entry, "entry", "blue"), (stop_loss, "stop", "red"),
        (target_2r, "2R", "green"), (target_3r, "3R", "darkgreen"),
    ):
        if value is not None:
            fig.add_hline(y=value, line_dash="dot", line_color=color,
                          annotation_text=name, annotation_position="right")
    fig.update_layout(title=title, xaxis_rangeslider_visible=False, height=450)
    return fig
...
def equity_curve(trades: pd.DataFrame, r_col: str = "net_r") -> go.Figure:
    ...
    if not trades.empty:
        ...
        fig.add_trace(go.Scatter(x=list(range(1, len(cum) + 1)), y=cum, mode="lines+markers", name="cumulative R"))
    fig.update_layout(title="Equity curve (cumulative R)", xaxis_title="trade #", yaxis_title="R", height=350)
    return fig
...
def r_multiple_histogram(trades: pd.DataFrame, r_col: str = "net_r") -> go.Figure:
    fig = go.Figure()
    if not trades.empty:
        fig.add_trace(go.Histogram(x=trades[r_col], nbinsx=30))
    fig.update_layout(title="R-multiple distribution", xaxis_title="R", height=350)
    return fig
...
def mfe_mae_scatter(points: pd.DataFrame) -> go.Figure:
    ...
    if not points.empty:
        fig.add_trace(go.Scatter(
            ...
            marker=dict(color=points["net_r"], colorscale="RdYlGn", showscale=True),
            text=points.get("symbol"),
        ))
    fig.update_layout(title="MFE vs MAE", xaxis_title="MAE (R)", yaxis_title="MFE (R)", height=400)
    return fig
