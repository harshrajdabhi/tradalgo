...
def test_shipped_config_loads_spec_values():
    ...
    # I3: every position knob must be one PositionManager/bounded_stop actually reads
    assert set(type(s.position_management).model_fields) == {
        "partial_exit_fraction", "partial_exit_at_r", "breakeven_after_partial", "runner_target_r",
        "stop_atr_min", "stop_atr_max"}
